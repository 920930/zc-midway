...
export const getRequest = async (
  ...
  const { data } = await makeHttpRequest(url, { dataType: 'json', method });
  return data;
};

const getAccessTokenFromWechatServer = async (): Promise<{
  access_token: string;
  expires_in: number;
}> => {
...
const getJsApiTicketFromWechatServer = async (
  access_token: string
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      const { access_token, expires_in } =
        await getAccessTokenFromWechatServer();
      resolve({
        token: access_token,
  ...
  return new Promise((resolve, reject) => {
    readFile(__dirname + '/wechatData.json', 'utf-8', async (err, data) => {
      ...
      try {
        ...
        const { ticket, expires_in } = await getJsApiTicketFromWechatServer(
          access.token
        );
        ret.access = access;
